...
from fastai.callback.fp16 import _copy_state

@patch
def before_fit(self:NonNativeMixedPrecision):
...
def opt_f(params,*args,**kwargs):
    ...
    return Adam(params,*args,**kwargs)
...
with no_random(seed): 
    ...
    print(type(learn.dls.device))
    learn.to_non_native_fp16()
    #learn.model.to(learn.dls.device)
    print(type(learn.dls.device))
    with learn.distrib_ctx(): learn.fit(10, lr=0.1)